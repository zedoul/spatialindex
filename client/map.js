(function() {
  ...
  */
   ...
   * ConvthreadMap
   * ==========
   * Wraps the Mapbox map. Responsible for plotting convthreads and notifying the rest of
   * the application about search marker movement.
   */
  var ConvthreadMap = function(center, searchRadius) {
    var self = this;
    ...
    var getConvthreadLayer = function(convthreads) {
      var features = convthreads.map(function(convthread) {
        var topic = convthread.topic;
        var color = hslToHex(0.2 * (1 - convthread.popularity), 0.8, 0.5);
        return {
          type: 'Feature',
          properties: {
            title: convthread.title,
            'marker-color': color,
            'marker-size': 'large',
            'marker-symbol': 'heart'
          },
          geometry: {
            type: 'Point',
            coordinates: [topic.lng, topic.lat]
          }
        };
      });

      var convthreadLayer = L.mapbox.featureLayer();

      convthreadLayer.on('mouseover', function(e) {
        e.layer.openPopup();
      });

      convthreadLayer.on('mouseout', function(e) {
        e.layer.closePopup();
      });

      convthreadLayer.setGeoJSON({
        type: 'FeatureCollection',
        features: features
      });

      return convthreadLayer;
    };

    this.plot = function(convthreads) {
      if (this.map.hasLayer(this.convthreadLayer)) {
        this.map.removeLayer(this.convthreadLayer);
      }
      this.convthreadLayer = getConvthreadLayer(convthreads);
      this.convthreadLayer.addTo(this.map);
    };
  ...
  var Searcher = function(prefs) {
    ...
    this.search = function(cb) {
      $.ajax({
        ...
        async: false,
        dataType: "json",
        data: {'lat': prefs.position.lat, 
               'lng': prefs.position.lng, 
               'radius': prefs.radius, 
               'tags': prefs.tags.join(), 
               'count': prefs.count},
        success: function(response) {
          cb(null, response.convthreads);
        }, 
  ...
  var init = function() {
    ...
    var map = new ConvthreadMap(prefs.position, prefs.radius);

    controls.on('search', function() {
      searcher.search(function(err, convthreads) {
        if (err) return alert(err);
        map.plot(convthreads);
      });
    });
  ...
  ConvthreadMap.prototype = Object.create(EventEmitter2.prototype);
